...
Update 机制指的是通过 HTTP Put 方法上传完整的资源对象，而 Kubernetes 使用完整的资源对象更新原来的资源对象。在更新对象时，Kubernetes 通过 resourceVersion 来检查是否发生冲突。

`metadata.resourceVersion` 可以代表资源的版本号，本质上是由 etcd 提供 modified index。在对象写入 etcd 后，resourceVersion 都会发生变化。

[**官方文档**](https://kubernetes.io/zh/docs/reference/using-api/api-concepts/#resource-versions)中指出，resourceVersion 仅仅只能用来比较是否相等，而没有大小之分，也就是说，不能按照 resourceVersion 的大小来判断对象的新旧。
...
Patch 机制指的是通过 HTTP Patch 方法上传增量的对象字段（除了 Server Side Apply 是上传完整对象），Kubernetes 会直接接受请求，将 patch 打到对象上，同时更新 resourceVersion。也就是说，Patch 机制（除了 Server Side Apply）默认没有冲突机制。
...
Merge Patch 直接上传需要修改的字段，用值为 null 表示删除一个字段。这也表明，Merge Patch 不允许单独修改数组中的某个元素，需要提供整个数组来进行修改，并且无法表示 null 值。
...
但是，使用 Strategic Merge Patch 方式需要 Kubernetes 知晓其数据结构的注释，因此只能应有与 Kubernetes 原生的资源对象。对于 Custom Resource 是无法使用的。

### 2.4 Server Side Apply

Kubernetes 1.16 之后，默认开启了 Server Side Apply 特性，在前面三种 Apply 方式的基础上，提供了冲突的机制。

#### 2.4.1 字段管理

Server Side Apply 将对象的每个字段都认为由一个 Manager 来管理。

使用 Server Side Apply 想要更新对象的某个字段时，如果该字段是由其他 Manager 管理时，就会触发 Conlict。也就表明，该字段的所有权是属于其他 Manager 的。
...
在任何对象更新的操作时，可以通过 fieldManager 查询操作标识 Manager（对于 Apply 操作是必须指定的）。

#### 2.4.2 Merge 策略

Server Side Apply 的请求与 Strategic Merge Patch 和 Merge Patch 类似。对于原生资源，Server Side Apply 使用 Strategic Merge Patch 的方式；对于非原生资源，使用 Merge Patch 的方式。

#### 2.4.3 Conflict 处理

当字段的更新触发 Conflict 时，有三种处理方式：

* 强制更新，转移所有权：使用 force 查询参数，再发送一次请求，这表明需要强制更新资源，并将更新的字段的 Manager 变更为当前。
...
`kubectl apply` 默认执行的是 client side apply，是基于对象的 last-applied-configuration annotation 进行前后对比。
...
`kubectl patch` 发送的是 Patch 请求，只需要上传增量的数据（等于说是由用户做 Diff 操作），默认使用 Strategic Merge Patch 类型。可以使用 `--type` 来指定 Patch 类型。
