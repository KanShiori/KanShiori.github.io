...
IAM 本质上是为了完成两个功能：
* Authentication - 身份认证，操作资源前确认请求者是 “谁”；
...
* 请求主体；
* 签名 - 由 SK 作为私钥，对 请求主体 + 元信息 + 时间戳 等信息进行加密，得到签名；

该凭证实现的作用：
* 身份验证 - AWS 服务端会查找对应的 AK 进行解密，从而验证是否是对应的 SK 加密的；
* 防止篡改 - AK 解密后得到摘要信息，通过对比摘要信息来检查请求主体是否被篡改；
* 防止重放攻击 - 请求中会带有 时间戳 信息，通过时间戳信息来判断请求有效期（默认 5min），防止重放攻击；

{{< admonition tip "AK/SK 的作用">}}
可以看到，AK 作用类似于了公钥，SK 作用类似于了私钥。
{{< /admonition >}}
...
对 Root User，因为其权限太大，所以日常使用不要使用 Root User，同时推荐对其开启 MFA，登陆时要求两步验证。

{{< admonition note "Root User 权限收到控制的情况">}}
有个例外，当使用 AWS Organization 限制时，某个 Account 的 Root User 会受到 Service Control Policy 限制。
{{< /admonition >}}

最开始创建 Account 时，先用 Root User 创建 Admin IAM User（作为顶级管理员），然后使用 Admin IAM User 去创建各个 IAM User。

### 2.2 IAM User

IAM User 访问 AWS 服务收到 IAM 的限制，通过 IAM Policy 来限制其访问权限，以满足最小权限原则（只有用户至少需要的权限）。

每个 IAM User 有着独立的 Username/Password 与 AK/SK。每个 IAM User 最多有两套 AK/SK，以方便轮换密钥：先停用当前使用的密钥，切换使用新的密钥，后删除旧的密钥。
...
Policy 定义了权限这个概念，通过将 Policy 附加到 User/Role，就可以限制其对 AWS 资源的访问权限。
...
User 有着长期使用的 Username/Password 与 AK/SK，并不适合临时提供给他人使用。而 Role 就是用于解决临时的授权操作，主要用于：
...
Assume Role 后，会在一定时间后过期，也支持手动撤销（Revoke）所有已经分配（不影响后续使用）。
...
执行 Assume Role 行为时，可以传递一个 Session Policy，作为本次 Assume Role 期间额外添加的 Policy。通过 Session Policy 实现一次性的约束。

### 4.3 Assume Role

Assume Role 本质上是调用 `sts:AssumeRole`/`sts:AssumeRoleWithWebIdentity` 相关 API，调用时会检查 Trust Relationship（实际上就是 IAM 检查 Policy）。

Assume 成功后，User/Service 会得到一个 AK/SK + Token 等信息，同时带有着超时时间。这些信息作为一个临时凭证，提供访问 AWS 资源的权限。而该临时凭证由 AWS STS 来颁发。
...
Permission Boundary 本质上就是为某个 Entity 指定一个全局的 Policy。当创建 User/Role 时，可以选择是否要带上 Permission Boundary（可选项）。
...
2. 基于资源策略显式允许 - 不存在显示拒绝情况下，如果 Resource-based Policy 是 Allow 拒绝，那么结果是 Deny；
