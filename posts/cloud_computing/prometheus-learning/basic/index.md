...
#### 1.2 Metric Name 与 Labels
Metric name 表示监控项的含义，Labels 表示一个指标的特定维度实例化
```metric
...
### 2.4 Summary
**`summary`** 为概率图，包含了各百分比数据的值，也就是百分比分布区间。

summary 包含三类数据：
* **bucket**：百分比范围数据的值，"quantile" 表示百分比
* **sum**：统计值的累计和
* **counter**：统计次数
...
* bucket
    ...
    quantile="0.5" - 50% 的 wal_fsync 操作耗时 0.012352463

    quantile="0.9" - 90% 的 wal_fsync 操作耗时 0.014458005

    quantile="0.99" - 99% 的 wal_fsync 操作耗时 0.017316173
* sum
...
Prometheus 拉取一个采集数据时，会自动在时序的基础上添加 `"job"` 与 `"instance"` 两个 label，用于识别被采集的目标。
```metric
